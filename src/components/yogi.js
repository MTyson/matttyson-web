...
const George = ({ className, data, onNav, flipCard, onFlip }) => {
	...
	const [contactPlaceholder, setContactPlaceholder] = useState("Drop me a line.");
	const submitForm = () => {
		var templateParams = {
			from_name: email,
		...
		<BackgroundImage Tag="section" className={className} fluid={data.yogiBg.childImageSharp.fluid} backgroundColor={`#E8D195`}
			...
			<div>
				<FlipCard show={flipCard} onClick={(e) => { e.preventDefault(); onFlip(); return false; }}>
					<div>
						<Img fixed={data.yogi.childImageSharp.fixed} style={{
							height: '444px', width: '301px',
						...
						<div className="flip-back" style={{
							...
							<Form style={{ display: 'flex', flexDirection: `column`, maxWidth: `100%`, justifyContent: `center`, alignItems: `center` }} onClick={(e) => { e.preventDefault(); e.stopPropagation(); return false; }}>
								<Form.Control as="textarea" placeholder={contactPlaceholder} value={body}
									onChange={(e) => setBody(e.target.value)} style={{ marginBottom: `.5em`, width: '250px' }}
									style={{ width:"95%", height:"250px"}}></Form.Control>
								<Form.Control type="email" placeholder="Your Email" onChange={(e) => setEmail(e.target.value)} value={email} 
								  style={{ marginBottom: `.5em`, width: `95%` }} />
